/**
 * ★ Properties Listing Page — Route Module with Server-Side Loader
 * 
 * ข้อมูลทรัพย์สินทั้งหมดถูกดึงจาก Firestore ใน `loader` ก่อนที่ React จะ render
 * ทำให้ HTML ที่ส่งกลับมามีรายการ properties ครบเลย (ไม่ต้องรอ Client-side JS)
 * 
 * NOTE: เดิมหน้านี้ดึงข้อมูลผ่าน useEffect → client-only rendering
 *       ตอนนี้เปลี่ยนเป็น loader → SSR (data พร้อมใน HTML แรก)
 */
import { createElement, useMemo, useState, useCallback, useRef, lazy, Suspense, useTransition } from 'react'
...
// ─── SEO Meta Tags (จะถูกใส่ใน HTML ตั้งแต่ Server render) ───────────────────
export function meta() {
  return [
    { title: "รายการประกาศ | SPS Property Solution" },
    { name: "description", content: "ค้นหาบ้าน คอนโด ทาวน์โฮม ให้เช่า-ขาย ในอมตะซิตี้ ชลบุรี" },
    { tagName: "link", rel: "canonical", href: "https://spspropertysolution.com/properties" },
  ]
}

// ─── Server-Side Loader ──────────────────────────────────────────────────────
// ดึงข้อมูลก่อน render — HTML จะมีรายการ properties ครบเลย
...
// บอก React Router ให้ hydrate ทันที แม้ loader ยังไม่เสร็จ (progressive enhancement)
...
const toSearchableText = (value) => {
  ...
  if (value && typeof value === 'object') {
    const preferred = [
      value.name, value.label, value.title, value.address,
      value.fullAddress, value.province, value.district,
    ]
          ...
          {options.map((opt) => (
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-2.5 text-sm transition-colors ${
                value === opt.value ? 'bg-blue-50 text-blue-900 font-bold' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
              {opt.label}
...
// ─── Component ───────────────────────────────────────────────────────────────
export default function Properties({ loaderData }) {
  // ★ ข้อมูลมาจาก loader — ไม่ต้อง useEffect ดึงเองแล้ว
  ...
  const { mapProperties, mapShowingMaxCaption } = useMemo(() => {
    ...
    return { mapProperties: withCoords.slice(0, 100), mapShowingMaxCaption: withCoords.length > 100 }
  ...
  const handleRemoveFilter = useCallback((filter) => {
    startTransition(() => {
      ...
      params.delete('page')
      switch (filter.type) {
        case 'keyword': setSearchQuery(''); setDebouncedKeyword(''); params.delete('q'); params.delete('search'); break
        case 'tag': updateFilters({ tag: '' }); params.delete('tag'); break
        case 'feature': updateFilters({ feature: '' }); params.delete('feature'); break
        case 'propertyType': updateFilters({ propertyType: '' }); params.delete('propertyType'); break
        case 'location': updateFilters({ location: '' }); params.delete('location'); break
        case 'propertySubStatus': updateFilters({ propertySubStatus: '' }); params.delete('status'); break
        case 'price': updateFilters({ priceMin: '', priceMax: '' }); params.delete('priceMin'); params.delete('priceMax'); break
        case 'bedrooms': updateFilters({ bedrooms: '' }); params.delete('bedrooms'); break
        case 'bathrooms': updateFilters({ bathrooms: '' }); params.delete('bathrooms'); break
        case 'area': updateFilters({ areaMin: '', areaMax: '' }); params.delete('areaMin'); params.delete('areaMax'); break
        case 'isRental': updateFilters({ isRental: null, subListingType: '' }); params.delete('type'); params.delete('listingType'); params.delete('subListingType'); break
        case 'project': updateFilters({ project: '' }); params.delete('project'); break
      }
      prevSearchParamsRef.current = params.toString()
